...
The factors `exchange`, `price_index`, and `inflation_to_2025` are attached for each year from `config/pnad_metadata.csv` when they are absent from the analytical files. Centralizing these constants prevents different analytical modules from using inconsistent values.
...
For each year and available income measure, the project reports the number of finite observations, the minimum positive value, the maximum value, the arithmetic mean, the median, the sample standard deviation, and the Gini coefficient. Nominal and adjusted quantities remain distinct columns.
